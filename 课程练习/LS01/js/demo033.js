(function(){
    //基本（原始）数据类型与引用（对象）类型的区别  赋值方式
    var str_a="a";
    var str_b=str_a;//原始类型直接访问值，是值赋值
    str_b="b";//str_b的值为”b“，而str_a的值仍然”a“
    ...
    obj_a={v:"c"};//注意：因为改的是整个对象，这里会在堆内存中创建一个新的对象值:{v:"c"},而现在的obj_b引用的是这个对象，
    //所以这里打印的obj_a依旧是{v:"c"},而obj_b是{v:"b"}(两者在内存中引用的是不同对象了)
    console.log(obj_a,obj_b);//{v:"c"} {v:"b"}
})
